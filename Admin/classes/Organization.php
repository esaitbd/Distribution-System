...
class Organization
{
    ...
    public function UPDATE_ORG($org, $org_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Update successful <a href='http://192.168.45.176:192/DS/sandbox/add_org.php'><span class='glyphicon glyphicon-backward'></span> </a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_ORG($org_id) {
        ...
        if($stmt->execute()) {
            header("location: http://192.168.45.176:192/DS/sandbox/add_org.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
