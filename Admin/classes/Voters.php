...
class Voters
{
    public function INSERT_VOTER($name, $course, $year, $stud_id) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sorry the Data you entered already exists in the database.</div>";
        } else {
            //Insert voter
            ...
            if($stmt->execute()) {
                echo "<div class='alert alert-success'> was inserted successfully.</div>";
            }
            $stmt->free_result();
        }
        return $stmt;
    ...
    public function UPDATE_VOTER($name, $course, $year, $stud_id, $voter_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'> was updated successfully.<a href='http://192.168.45.176:192/DS/sandbox/add_voters.php'><span class='glyphicon glyphicon-backward'></span></a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_VOTER($voter_id) {
        ...
        if($stmt->execute()) {
            header("location: http://192.168.45.176:192/DS/sandbox/add_voters.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
