...
class Nominees
{
    ...
    public function INSERT_NOMINEE($org, $pos, $name, $course, $year, $stud_id) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sorry the Data you entered already exist in the database</div>";
        } else {
            //Insert nominee
    ...
    public function UPDATE_NOMINEE($org, $pos, $name, $course, $year, $stud_id, $nom_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Update successful <a href='http://192.168.45.176:192/DS/sandbox/add_nominees.php'><span class='glyphicon glyphicon-backward'></span> </a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_NOMINEE($nom_id) {
        ...
        if($stmt->execute()) {
            header("location: http://192.168.45.176:192/DS/sandbox/add_nominees.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
