...
class Position
{
    public function INSERT_POS($org, $pos) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sorry the Data you entered is already inserted in the database.</div>";
        } else {
    ...
    public function UPDATE_POS($org, $pos, $pos_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'> was updated successfully.<a href='http://192.168.45.176:192/DS/sandbox/add_pos.php'><span class='glyphicon glyphicon-backward'></span></a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_POS($pos_id) {
        ...
        if($stmt->execute()) {
            header("location: http://192.168.45.176:192/DS/sandbox/add_pos.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
