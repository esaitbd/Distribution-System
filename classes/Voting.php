...
class Voting
{
    ...
    public function VOTE_NOMINEE($org, $pos, $candidate_id, $voters_id) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sorry already the item you have taken.</div>";
        } else {
            //Vote successful.
            ...
            if($stmt->execute()) {
                echo "<div class='alert alert-success'>DIU Distribution Submitted successful.</div>";
            }
            $stmt->free_result();
        }
        return $stmt;
